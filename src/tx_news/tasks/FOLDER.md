...
架构（≤3行）：
- `celery_app.py` 定义 Celery app 与任务注册策略（并包含 v2.2 所需的 beat_schedule；queue worker 不再依赖过期策略；并为队列工人配置软/硬超时防止卡住；并默认抑制 httpx/qdrant 的 HTTP Request 噪声日志）。
...
- `news_queue.py` 维护分析优先队列（按发布时间最新优先，24h 过期清理），并由 Celery 定时 `queue_worker_task` 拉取队列触发 `pipeline.analyze`（队列工人对新入库 canonical 标记 `is_new_canonical`，确保深分析能被触发；默认非阻塞并带运行时上限以避免任务堆积/卡死）。
