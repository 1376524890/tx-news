...
| `settings.py` | 配置入口 | 环境变量（含 `llm.chat`/`llm.deep` 分流）+ `config/` 文件加载与默认值；并提供 `TXNEWS_CHAT_ALLOW_DEEP_FALLBACK`（对话是否允许回退到 `llm.deep`）与 `TXNEWS_ALLOW_FULL_TEXT`（内部 KB 全文开关）。 |
