<!-- Input: 配置 UI（前端构建产物）+ Redis/Postgres/NATS/Infra 状态 -->
<!-- Output: 8001 端口的配置服务 + 监控页与监控 API -->
...
架构（≤3行）：
- 独立端口（默认 `8001`）提供“配置界面”与监控页面，替代原先对外暴露的管理台 UI。
- 通过 cookie 标识用户，并将每个用户的在线 LLM 配置写入 Redis（用于 chat 按用户分摊成本）。
- 静态资源来自 `apps/web/dist_admin/`（Vite admin 构建产物）；监控页为内置 HTML（`/monitor`）。
...
| `main.py` | 服务入口 | FastAPI：提供配置 UI、`/api/config` 读写接口与 `/api/monitor` 监控数据。 |
